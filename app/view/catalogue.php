...
if(isset($_SESSION['id']) && isset($_SESSION['role']) && !empty($_SESSION['id']) && !empty($_SESSION['role'])){
    if($_SESSION['role'] == 1){
        $getButtons = '<a href="http://localhost/youdemy2/public/admin/dashboard" class="px-4 py-2 bg-indigo-600 text-white rounded-full hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-600/20">Dashboard</a>';
    }elseif($_SESSION['role'] == 2){
        $getButtons = '<a href="http://localhost/youdemy2/public/teacher/dashboard" class="px-4 py-2 bg-indigo-600 text-white rounded-full hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-600/20">Dashboard</a>';
    }else{
    ...
    $getButtons = '<a href="login/login" class="px-4 py-2 text-gray-600 hover:text-indigo-600 transition-colors">Log In</a>
            ...
            <div class="flex items-center justify-between">
                ...
                <nav class="hidden md:flex items-center space-x-8">
                    ...
                    <?php
                     ...
                     ?>
                    <a href="home" class="text-gray-600 hover:text-indigo-600 transition-colors">About</a>
                    <a href="home" class="text-gray-600 hover:text-indigo-600 transition-colors">Contact</a>
                </nav>
    ...
    <header class="bg-white shadow-md py-4">
        <div class="container mx-auto px-6 md:px-12 flex justify-between items-center space-x-6">
            ...
            <div class="flex overflow-x-auto space-x-4 py-2 px-4 sm:px-6 custom-scrollbar">
                <a href="catalogue" class="px-6 py-2 bg-indigo-100 rounded-lg text-indigo-700 shadow-md hover:bg-indigo-200 transition duration-300 flex-shrink-0">
                    All
                </a>
                ...
                <?php
                    ...
                    if($getCategories != null && $getCategories->rowCount() > 0){
                        foreach($getCategories as $categorie){
                            echo '<a href="catalogue?catalogue='.$categorie['id_categorie'].'&page=1" class="px-6 py-2 bg-gray-100 rounded-lg text-gray-700 shadow-md hover:bg-gray-200 transition duration-300 flex-shrink-0">
                                '.$categorie['name'].'
                            </a>';
                        }
                    }
...
if ($getCours != null && count($getCours['data']) > 0) {
                ...
                echo '<div class="bg-white shadow-xl rounded-lg overflow-hidden transform transition duration-300 hover:scale-105 hover:shadow-2xl max-w-sm">
                        ...
                        <div class="p-6">
                            ...
                            <div class="flex justify-between items-center">
                                <a 
                                    href="http://localhost/youdemy2/public/cour?cours='.$cours['id_cours'].'" 
                                    target="_blank" 
        ...
        <div class="mt-12 flex justify-center space-x-4">
            <?php
                ...
                if(isset($getPages) && $getPages == 1){
                    echo '<a href="catalogue?page=1" class="px-6 py-2 bg-indigo-600 text-white rounded-lg shadow-md hover:bg-indigo-700 focus:outline-none transition duration-200">1</a>';
                }elseif(isset($getPages) && $getPages > 1){

                    for($i = 1; $i <= $getPages; $i++){
                        echo '<a href="catalogue?page='.$i.'" class="px-6 py-2 bg-indigo-600 text-white rounded-lg shadow-md hover:bg-indigo-700 focus:outline-none transition duration-200">'.$i.'</a>';
                    }
