...
class categorie {
    ...
    public static function getCategories($conn) {
        $getData = $conn->prepare("SELECT categorie.id_categorie, categorie.name, COUNT(cours.id_cours) AS total 
                                    FROM categorie 
                                    LEFT JOIN cours ON categorie.id_categorie = cours.id_categorie 
                                    GROUP BY categorie.id_categorie, categorie.name
                                    ");
    ...
    public function addCategorie($name, $conn) {
        $this->name = $name;

        $addCategorie = $conn->prepare("INSERT INTO categorie (name) VALUES (:name)");
