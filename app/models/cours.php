...
<?php
    ...
    class cours{
        ...
        public static function getCours($conn, $offSet = 0, $getCategorie = '', $search = '') {
            // Ensure that $offSet is an integer and defaults to 0 if invalid
            $offSet = is_numeric($offSet) ? (int)$offSet : 0;
        
            // Get all data (to count the total number of rows for pagination)
            $getAllData = $conn->prepare("SELECT COUNT(*) FROM cours 
                                          INNER JOIN users ON cours.id_user = users.id_user 
            ...
            if (!empty($search)) {
                $getSearch = '%' . $search . '%';
                $getData = $conn->prepare("SELECT * FROM cours 
                                          INNER JOIN users ON cours.id_user = users.id_user 
                                          INNER JOIN categorie ON cours.id_categorie = categorie.id_categorie 
                                          WHERE id_approved = '1' 
                                          AND (titre LIKE :search OR description LIKE :search) 
                                          LIMIT 6 OFFSET :offset");
                $getData->bindParam(":search", $getSearch, PDO::PARAM_STR);
            } elseif (!empty($getCategorie)) {
                $getData = $conn->prepare("SELECT * FROM cours 
                                          INNER JOIN users ON cours.id_user = users.id_user 
                                          INNER JOIN categorie ON cours.id_categorie = categorie.id_categorie 
                                          WHERE id_approved = '1' AND cours.id_categorie = :categorie 
                                          LIMIT 6 OFFSET :offset");
            ...
            // Bind the OFFSET parameter to an integer
            ...
            // Fetch the result count for pagination
            $totalRows = $getAllData->fetchColumn();
            
            if ($getData->rowCount() > 0) {
                return [
                    "data" => $getData->fetchAll(PDO::FETCH_ASSOC),
                    "pages" => ceil($totalRows / 6),
                ];
            } else {
                return null;
            }
        ...
        public function getSingleCour($getID, $conn) {
            $this->id_cours = $getID;
            $getCour = $conn->prepare("SELECT * FROM cours 
                                       INNER JOIN users ON cours.id_user = users.id_user 
                                       WHERE id_cours = :getID");
        ...
        public function mycours($getID, $conn) {
            $this->id_cours = $getID;
            $getMyCours = $conn->prepare("SELECT * FROM mycours 
                                          INNER JOIN cours ON mycours.id_cours = cours.id_cours 
                                          INNER JOIN categorie ON cours.id_categorie = categorie.id_categorie 
        ...
        public function checkMyCour($user, $cours, $conn) {
            $this->id_cours = $cours;
            $this->id_user = $user;
            $getCount = $conn->prepare("SELECT * FROM mycours 
                                        WHERE id_cours = :cours AND id_user = :user");
        ...
        public function joinCour($user, $cours, $conn) {
            $this->id_user = $user;
            $this->id_cours = $cours;
            $join = $conn->prepare("INSERT INTO mycours(id_cours, id_user, date_join) 
                                    VALUES(:cours, :user, CURRENT_TIMESTAMP)");
        ...
        public function myStudentCount($getID, $conn) {
            $myStudentCount = $conn->prepare("SELECT COUNT(DISTINCT users.id_user) AS total 
                                              FROM users 
                                              INNER JOIN mycours ON users.id_user = mycours.id_user 
                                              INNER JOIN cours ON cours.id_cours = mycours.id_cours 
                                              WHERE cours.id_user = :getID");
        ...
        public function getBestCours($getID, $conn) {
            $getCours = $conn->prepare("SELECT titre, COUNT(cours.id_cours) AS joinCount 
                                       FROM cours 
                                       INNER JOIN mycours ON cours.id_cours = mycours.id_cours 
                                       WHERE cours.id_user = :getID 
        ...
        public function recentActivities($getID, $conn) {
            $this->id_user = $getID;
            $getActivities = $conn->prepare("SELECT nom, prenom, titre, date_join 
                                            FROM users 
                                            INNER JOIN mycours ON users.id_user = mycours.id_user 
                                            INNER JOIN cours ON mycours.id_cours = cours.id_cours 
                                            WHERE cours.id_user = :getID 
                                            ORDER BY date_join DESC LIMIT 3");
        ...
        public function getAllMyStudents($getID, $conn, $filter) {
            ...
            if ($filter) {
                $getUsers = $conn->prepare("SELECT nom, prenom, email, titre 
                                           FROM users 
                                           INNER JOIN mycours ON users.id_user = mycours.id_user 
                                           INNER JOIN cours ON cours.id_cours = mycours.id_cours 
                                           WHERE cours.id_user = :getID AND cours.id_cours = :cour");
        ...
        protected function addCours($id, $titre, $desc, $content, $img, $user, $categorie, $type, $conn) {
            ...
            $addCours = $conn->prepare("
                INSERT INTO cours(titre, description, content, id_approved, id_user, id_categorie, imgSrc, date_create, typeContent) 
                VALUES(:titre, :desc, :content, FALSE, :id_user, :id_categorie, :imgSrc, CURRENT_DATE, :type)
            ");
            ...
            if ($addCours->execute()) {
                return $conn->lastInsertId("cours_id_cours_seq");
            } else {
                return false;
            }
        }
        ...
        public function removeCour($cour, $user, $conn){
            ...
            $removeTagsCours = $conn->prepare("DELETE FROM tags_cours WHERE id_cours = :cours");
            $removeTagsCours->bindValue(":cours", $this->id_cours, PDO::PARAM_INT);
            $removeTagsCours->execute();
            
            $removeCours = $conn->prepare("DELETE FROM cours WHERE id_cours = :cours AND id_user = :user");
            $removeCours->bindValue(":cours", $this->id_cours, PDO::PARAM_INT);
            $removeCours->bindValue(":user", $this->id_user, PDO::PARAM_INT);
                    ...
                    echo '<section class="bg-white rounded-2xl shadow-xl p-8 mb-8 overflow-hidden">
                            ...
                            <div class="aspect-video bg-gray-900 rounded-xl overflow-hidden relative group">
                                <video controls class="w-full h-full object-cover">
                                    <source src="../../public/videos/'.$content['content'].'" type="video/mp4">
        ...
        {
            $getCourses = $conn->prepare("SELECT * FROM cours 
                                          INNER JOIN users ON cours.id_user = users.id_user 
                                          INNER JOIN categorie ON categorie.id_categorie = cours.id_categorie 
                                          WHERE id_approved = FALSE");
        ...
        {
            $this->id_cours = $id;
            $accept = $conn->prepare("UPDATE cours SET id_approved = true WHERE id_cours = :getID");
        ...
        $deleteTagsCours = $conn->prepare("DELETE FROM tags_cours WHERE id_cours = :getID");
        $deleteTagsCours->bindParam(":getID", $this->id_cours);
        $deleteTagsCours->execute();
        ...
        {
            $getCours = $conn->prepare("
                SELECT cours.*, MAX(categorie.name) AS tt, COUNT(mycours.id_cours) AS total
                FROM cours
                LEFT JOIN mycours ON mycours.id_cours = cours.id_cours
                INNER JOIN categorie ON cours.id_categorie = categorie.id_categorie
                GROUP BY cours.id_cours, cours.id_categorie
                ORDER BY total DESC
                LIMIT 3
