...
class tags {
    ...
    public function addTags($conn) {
        if ($this->id_tag === null) {
            ...
            if ($addTags->execute()) {
                return $conn->lastInsertId('tags_id_tag_seq'); // PostgreSQL specific sequence usage
            } else {
                return false;
            }
        } else {
    ...
    public static function getTags($conn) {
        $getTags = $conn->prepare("SELECT id_tag, name FROM tags");
        if ($getTags->execute()) {
            return $getTags;
        } else {
            return null;
    ...
    public static function getSelectTags($conn) {
        $getTags = $conn->prepare("SELECT DISTINCT name FROM tags");
        if ($getTags->execute()) {
            return $getTags;
        } else {
            return null;
